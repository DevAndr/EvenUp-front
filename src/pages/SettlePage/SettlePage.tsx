...
import type { Group, Member, Expense, Transaction, DebtResult, BalanceMap } from "@/types/types.ts";

// ─── API ──────────────────────────────────────────────────────────────────────
// const fetchGroup  = (id: string): Promise<Group> =>
//   axios.get(`/api/groups/${id}`).then(r => r.data);
// const markSettled = (data: MarkSettledPayload): Promise<void> =>
//   axios.post("/api/settlements", data).then(r => r.data);
...
const mockGroup: Group = {
    ...
    expenses: [
        { id: "e1", description: "Отель",     amount: 18000, paidBy: "1", category: "home",  date: "2025-02-20T10:00:00Z", splitWith: ["1","2","3","4","5"] },
        { id: "e2", description: "Ужин",      amount: 6400,  paidBy: "2", category: "food",  date: "2025-02-20T20:00:00Z", splitWith: ["1","2","3","4","5"] },
        { id: "e3", description: "Такси",     amount: 1800,  paidBy: "1", category: "taxi",  date: "2025-02-19T14:00:00Z", splitWith: ["1","2","3","4"] },
        { id: "e4", description: "Продукты",  amount: 3200,  paidBy: "3", category: "grocery",date: "2025-02-21T11:00:00Z", splitWith: ["1","2","3","4","5"] },
        { id: "e5", description: "Эрмитаж",   amount: 2500,  paidBy: "4", category: "music", date: "2025-02-21T15:00:00Z", splitWith: ["1","2","3","4","5"] },
        { id: "e6", description: "Билеты",    amount: 24000, paidBy: "1", category: "plane", date: "2025-02-18T08:00:00Z", splitWith: ["1","2","3","4","5"] },
    ],
};
...
function calcDebts(members: Member[], expenses: Expense[]): DebtResult {
    ...
    type DebtEntry = { uid: string; name: string; isYou: boolean; amount: number };
...
interface AvatarProps { name: string; isYou?: boolean; size?: string; text?: string }
function Avatar({ name, isYou = false, size = "w-10 h-10", text = "text-sm" }: AvatarProps) {
    return (
...
interface SbpCopySheetProps {
    open: boolean;
    onClose: () => void;
    transaction: Transaction | null;
}

function SbpCopySheet({ open, onClose, transaction }: SbpCopySheetProps) {
    const [copied, setCopied] = useState<boolean>(false);
    if (!transaction) return null;

    const phone = "+7 (999) 123-45-67"; // из профиля получателя
        ...
        <Sheet open={open} onOpenChange={onClose}>
            <SheetContent side="bottom" className="bg-[#161616] border-t border-[#2a2a2a] rounded-t-3xl pb-10">
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="bg-[#1e1e1e] border border-[#2a2a2a] rounded-2xl p-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <Button variant="ghost" size="icon" onClick={handleCopy}
                                    className={`w-9 h-9 rounded-xl border transition-all ${copied ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-400" : "bg-[#2a2a2a] border-[#333] text-zinc-400 hover:text-zinc-200"}`}
                            >
                    ...
                    <Button onClick={onClose} className="w-full bg-indigo-500 hover:bg-indigo-600 rounded-2xl h-12 font-bold">
                        Готово
                    </Button>
                </div>
            </SheetContent>
...
function ConfirmDialog({ open, onClose, onConfirm, transaction, loading }: ConfirmDialogProps) {
    if (!transaction) return null;
    return (
            ...
            <DialogContent className="bg-[#161616] border border-[#2a2a2a] rounded-3xl max-w-sm mx-4 p-6">
                ...
                <div className="mt-3 flex flex-col gap-4">
                    ...
                    <p className="text-sm text-zinc-500 text-center leading-relaxed">
                        Отметить этот перевод как выполненный? Долг будет закрыт для всех участников.
                    </p>
...
interface TransactionCardProps {
    ...
    onSbp: (tx: Transaction) => void;
}

function TransactionCard({ transaction, settled, onSettle, onSbp }: TransactionCardProps) {
    ...
    return (
        <div className={`rounded-2xl border p-4 transition-all ${
            settled ? "bg-[#141414] border-[#1e1e1e] opacity-50"
                : youInvolved ? "bg-indigo-500/[0.06] border-indigo-500/20"
            ...
            <div className="flex items-center justify-between mb-4">
                <div className="flex flex-col items-center gap-1.5">
                    ...
                    <span className="text-xs text-zinc-500 font-medium">{transaction.from.isYou ? "Вы" : transaction.from.name}</span>
                    <span className="text-[10px] text-zinc-700">должен</span>
                </div>

                <div className="flex flex-col items-center gap-1 flex-1 px-3">
                    <div className={`font-extrabold text-xl ${settled ? "text-zinc-600" : youInvolved ? "text-indigo-400" : "text-zinc-300"}`}>
                ...
                <div className="flex flex-col items-center gap-1.5">
                    ...
                    <span className="text-xs text-zinc-500 font-medium">{transaction.to.isYou ? "Вам" : transaction.to.name}</span>
            ...
            {settled ? (
                ...
                <div className="flex gap-2">
                    {youDebtor && (
                        <Button onClick={() => onSbp(transaction)} variant="ghost"
                    ...
                    <Button onClick={() => onSettle(transaction)}
...
export default function SettlePage() {
    ...
    const [sbpTx, setSbpTx]                 = useState<Transaction | null>(null);
    ...
    const handleConfirm = (): void => {
        ...
        // await markSettled({ groupId: id!, fromUserId: confirmTx.from.uid, toUserId: confirmTx.to.uid, amount: confirmTx.amount });
        setTimeout(() => {
    ...
    return (
      ...
      `}</style>
            ...
            {/* My balance banner */}
            {myBalance !== 0 && (
                <div className={`rounded-2xl p-4 mb-5 border ${myBalance > 0 ? "bg-emerald-400/[0.07] border-emerald-400/20" : "bg-red-400/[0.07] border-red-400/20"}`}>
                    <div className="flex items-center justify-between">
                        ...
                        <div className="text-3xl">{myBalance > 0 ? "💰" : "💸"}</div>
                    </div>
                </div>
            )}
                ...
                <div className="flex flex-col gap-3 fade-up">
                    <div className="flex items-center justify-between mb-1">
                        ...
                        <div className="flex gap-1">
                            {transactions.map(t => (
                                <div key={t.id} className={`h-1.5 w-6 rounded-full transition-all duration-300 ${settled.has(t.id) ? "bg-emerald-500" : "bg-[#2a2a2a]"}`} />
                            ))}
                        </div>
                    </div>
                    {transactions.map(tx => (
            ...
            <ConfirmDialog open={!!confirmTx} onClose={() => setConfirmTx(null)} onConfirm={handleConfirm} transaction={confirmTx} loading={confirmLoading} />
            <SbpCopySheet  open={!!sbpTx}    onClose={() => setSbpTx(null)}    transaction={sbpTx} />
        </div>
    );
}