...
// ─── API ──────────────────────────────────────────────────────────────────────
// const createGroup = (data) => axios.post("/api/groups", data).then(r => r.data);
            ...
            {Array.from({ length: total }).map((_, i) => (
                <div
                    ...
                    className={`h-[3px] rounded-full transition-all duration-300 ${
                        i <= current ? "bg-indigo-500" : "bg-zinc-800"
                    } ${i === current ? "w-6" : "w-2"}`}
                />
            ))}
        </div>
...
function EmojiPickerSheet({ value, onChange }) {
...
function MemberChip({ name, onRemove, isYou }) {
    return (
...
function SplitTypeCard({ type, selected, onClick }) {
    return (
        <button
        ...
        >
            ...
            <div className={`font-bold text-sm ${selected ? "text-indigo-300" : "text-zinc-300"}`}>
                {type.label}
            </div>
...
export default function CreateGroupPage() {
    ...
    const removeMember = (memberName) => setMembers(prev => prev.filter(m => m !== memberName));
    ...
    const handleCreate = () => {
        const payload = { emoji, name, description, members, splitType };
    ...
    return (
      ...
      `}</style>
                    ...
                    <div className="flex gap-2.5 mb-7">
                        {SPLIT_TYPES.map(t => (
                            <SplitTypeCard
                                key={t.id}
                                type={t}
                                selected={splitType === t.id}
                                onClick={setSplitType}
                            />
                        ))}
                    </div>
            ...
            <div className="fixed bottom-0 left-0 right-0 max-w-[480px] mx-auto px-4 pb-7 pt-8 bg-gradient-to-t from-[#0f0f0f] via-[#0f0f0f]/90 to-transparent">
                ...
                >
                    {step < 2 ? (
                        <>Далее <ChevronLeft size={18} className="rotate-180 ml-1" /></>
                    ) : (
